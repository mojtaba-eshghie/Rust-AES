...
// using static definition for SBOX and I_SBOX
...
static I_SBOX: [[u8;16];16] = [ [0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb],
...
fn substitute(byte: u8, encryption: bool) -> u8 {
    ...
    if encryption == true {
        ...
        return I_SBOX[upper_nibble][lower_nibble];
    }
    */
}
...
fn encrypt_AES128(aes128: &AES128, bytes: &[u8]) -> Vec<u8> {
    ...
    if bytes.len()%16!=0 {
        panic!("Bytes argument length has to be multiple of 16!");
    }
    ...
    //println!("checkpoint last checkpoint in encrypt_AES128");
...
fn main() {
    ...
    //println!("The whole input stream: {:?}", data);

    // allocating the from stdin vector
    let mut key_bytes: [u8; 16] = data[0..16].try_into().unwrap();
    //println!("key bytes: {:?}", key_bytes);
    //println!("data bytes: {:?}", &data[16..32]);
    

    

    // TODO: only data from &data[16..] should be used as plaintext to encrypt. However, I am not sure how to slice
    // the vector in a way does not require copy in memory!
    let mut plaintext = Vec::new();
    plaintext.extend_from_slice(&data[16..]);
    ...
    let aes: AES128 = AES128::new(&key_bytes);
    
    

    
    let mut output_bytes:Vec<u8> = (aes.encrypt)(&aes, &plaintext);
    //println!("Encrypted bytes are: {:?}", output_bytes);


    /*
    // TODO: delete this for production;
    // Just for the sake of banchmarking
    benchmarking::warm_up();

    let bench_result = benchmarking::measure_function(move |measurer| {
        measurer.measure(|| {     
            // put the code you want to be measured here!
    ...
    println!("The desired operation inside measurer.measure took {:?}!", bench_result.elapsed());
    */


    //println!("{:?}", output_bytes)

    // Writing the cipher to stdout
